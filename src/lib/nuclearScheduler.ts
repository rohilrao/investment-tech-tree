import { TechTree, InvestTechTreeNode, InvestTechTreeEdge } from './types';
...
export class NuclearScheduler {
  private nodes: Record<string, InvestTechTreeNode>;
  private edges: InvestTechTreeEdge[];
  ...
  private buildSuccessorMap(): Record<string, string[]> {
    ...
    this.edges.forEach((edge) => {
      if (edge.source && edge.target && succ[edge.source]) {
  ...
  private getInitialProb(node: InvestTechTreeNode): number {
  ...
  private findCriticalPath(
    nodeId: string,
    currentNodes: Record<string, SimulationNode>,
  ): [number, number] {
    ...
    prereqIds.forEach((prereqId) => {
      const [prereqTime, prereqProb] = this.findCriticalPath(
        prereqId,
        currentNodes,
      );
    ...
    const maxPrereqTime =
      prereqTimes.length > 0 ? Math.max(...prereqTimes) : 0.0;
  ...
  private calculateDiscountedMwh(deploymentYear: number): number {
    ...
    for (let i = 0; i < YEARS_OF_OPERATION; i++) {
      ...
      if (year > CURRENT_YEAR) {
        totalDiscountedMwh +=
          annualMwh / Math.pow(1 + DISCOUNT_RATE, year - CURRENT_YEAR);
      }
    }
  ...
  private calculatePathwayMwh(
    nodesToSim: Record<string, SimulationNode>,
    conceptIds: string[],
  ): number {
    ...
    conceptIds.forEach((conceptId) => {
      const [timeToDeploy, probOfSuccess] = this.findCriticalPath(
        conceptId,
        nodesToSim,
      );
  ...
  runSimulation(yearsToSimulate: number = 20): SimulationResults {
    ...
    Object.entries(this.nodes).forEach(([nodeId, node]) => {
      ...
      if (node.trl_projected_5_10_years) {
        ...
        try {
          const trlVal = parseFloat(
            (node.trl_current || '1').split('-')[0].split(' ')[0],
          );
    ...
    for (
      ...
      year < CURRENT_YEAR + yearsToSimulate;
      year++
    ) {
        ...
        if (isActive) {
          ...
          if (node.time_remaining > 0) {
            ...
            const riskReductionPerYear =
              (1 - this.getInitialProb(this.nodes[nodeId])) / node.initial_time;
      ...
      Object.entries(simNodes).forEach(([nodeId, node]) => {
        ...
        const baselineMwh = this.calculatePathwayMwh(
          simNodes,
          affectedConcepts,
        );
        ...
        const riskReductionPerYear =
          (1 - this.getInitialProb(this.nodes[nodeId])) /
          tempNodes[nodeId].initial_time;
        tempNodes[nodeId].prob_of_success += riskReductionPerYear;

        // Calculate accelerated MWh
        const acceleratedMwh = this.calculatePathwayMwh(
          tempNodes,
          affectedConcepts,
        );
...
export function calculateSummaryStats(
  ...
  const maxImpact = Math.max(
    ...Object.values(impactData).map((yearlyImpacts) =>
      Object.values(yearlyImpacts).length > 0
        ? Math.max(...Object.values(yearlyImpacts))
        : 0,
    ),
  );

  const currentYear = new Date().getFullYear();
  const currentOpportunities = Object.values(impactData).filter(
    (techData) => (techData[currentYear] || 0) > 0,
  ).length;
...
export function convertDataToTechTree(data: {
  nodes: Array<{ id: string; data: InvestTechTreeNode }>;
  edges: InvestTechTreeEdge[];
}): { graph: { nodes: InvestTechTreeNode[]; edges: InvestTechTreeEdge[] } } {
  return {
    graph: {
      nodes: data.nodes.map(
        (node: { id: string; data: InvestTechTreeNode }) => ({
          id: node.id,
          label: node.data.label,
          type: node.data.type,
          references: node.data.references,
